...
#pragma region ImGui 
	BackendHandler::imGuiCallbacks.push_back([&]() {
		...
		if (ImGui::CollapsingHeader("Sphere Mesh Settings"))
		{	
			if (ImGui::Checkbox("Sphere ON", &SphereON))
			{
				SphereON = true;
				SphereOFF = false;	
			}
			if (ImGui::Checkbox("Sphere OFF", &SphereOFF))
			{
				SphereON = false;
				SphereOFF = true;
			}
			
			if (ImGui::SliderFloat("Sphere Scale", &SphereScale, 0.0f, 10.0f))
			{
			}
			if (ImGui::SliderFloat("Sphere X Position", &SphereX, -20.0f, 20.0f))
			{
				illuminationBuffer->SetSun(SphereX, SphereY, SphereZ);
			}
			if (ImGui::SliderFloat("Sphere Y Position", &SphereY, -20.0f, 20.0f))
			{
				illuminationBuffer->SetSun(SphereX, SphereY, SphereZ);
			}
			if (ImGui::SliderFloat("Sphere Z Position", &SphereZ, -20.0f, 20.0f))
			{
				illuminationBuffer->SetSun(SphereX, SphereY, SphereZ);
			}
...
// We need to tell our scene system what extra component types we want to support
	...
	blackMat->Shader = gBufferShader;
	blackMat->Set("s_Diffuse", black);
	...
	VertexArrayObject::sptr vao1 = ObjLoader::LoadFromFile("models/sphere.obj");
	GameObject sphere = scene->CreateEntity("sphere");
	{
		sphere.emplace<RendererComponent>().SetMesh(vao1).SetMaterial(blackMat);
		sphere.get<Transform>().SetLocalPosition(0.0f, 0.0f, 0.0f);
		sphere.get<Transform>().SetLocalRotation(0.0f, 0.0f, 0.0f);
	}
...
#pragma endregion
		...
		if ((time.CurrentFrame - lastfpsTime) >= 1.0 / 90.0) {//HZ limit
			...
			sphere.get<Transform>().SetLocalPosition(SphereX, SphereY, SphereZ);
			sphere.get<Transform>().SetLocalScale(SphereScale, SphereScale, SphereScale);
			illuminationBuffer->PointSunRef()._lightPos,SphereX, SphereY, SphereZ, 0.0f;
			cameraObject.get<Transform>().SetLocalPosition(P1.getPosition() + glm::vec3(0, 8, 12));
...
#pragma region sort and render and registry something exolol
	...
	// Iterate over all the behaviour binding components
		...
		glm::mat4 lightViewMatrix = glm::lookAt(glm::vec3(-illuminationBuffer->PointSunRef()._lightPos), glm::vec3(), glm::vec3(0.0f, 0.0f, 1.0f));
